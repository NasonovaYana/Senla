...
$str = 'qwerty';
// todo ИСПРАВЛЕНО (потому что при отсутствии длины вырежет до конца строки, что, по факту, и требуется)  здесь длину не обязаительно указывть. Вопрос почему?
...
// todo ИСПРАВЛЕНО имелось в виду, что можно в функцию передать 2 строки, и замена будет в соответвии с порядком символов.
// strtr($str, 'abc', '123');
...
unset($arr);
// todo ИСПРАВЛЕНО length = 1 - это значение по умочанию, его не обязательно указывать
$arr = str_split($str);
var_dump($arr);
echo '<br><br>';
...
$str = '12345678';
// todo ИСПРАВЛЕНО по заданию не нужно было переопределять разделитель дробной части
...
//            xxxxx
$str = 'x';
// todo ИСПРАВЛЕНО у пирамиды должно быть 9 рядов
for ($i = 1; $i<10; $i++){
...
//999999999
for ($i = 1; $i<10; $i++){
    // todo ИСПРАВЛЕНО зачем тебе $str, если она содержит тоже самое, что и $i?
