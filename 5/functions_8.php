...
// TODO: ИСПРАВЛЕНО Если в качестве аргумента format передать 'L', то функция date сразу вернет true, если год високосный.
    $daysFeb = date('L', $date);
// TODO: ИСПРАВЛЕНО проверку на isset надо делать до первого обращения к $_POST['yearType'].
//
//    if ($daysFeb == 29) {
...
// TODO: ИСПРАВЛЕНО $wd не очень название для переменной
    $weekDay = date('N', mktime(0, 0, 0, (int)$date[1], (int)$date[0], (int)$date[2]));
    echo $week[$weekDay];
}
...
// TODO: ИСПРАВЛЕНО если выбрать дату в декабре, то выводится неверный знак Зодиака
if (isset($_POST['birthZodiacDate'])) {
    ...
    if ($zodiacDate[0] <= $zodiacNext[$index]) {
        ...
        $index++;
        if ($index == 13 and (int)$zodiacDate[0] > 21) {
            echo $zodiac[1] . '<br>';
        } elseif ($index == 13 and (int)$zodiacDate[0] <= 21) {
            echo $zodiac[12] . '<br>';
        } else {
            echo $zodiac[$index] . '<br>';
        }
...
function getZodiacalSign($month, $day) {
    $signs = ["Козерог", "Водолей", "Рыбы", "Овен", "Телец", "Близнецы", "Рак", "Лев", "Девы", "Весы", "Скорпион", "Стрелец", "Козерог"];
    $signsStart = [1=>21, 2=>20, 3=>20, 4=>20, 5=>20, 6=>20, 7=>21, 8=>22, 9=>23, 10=>23, 11=>23, 12=>23];
    return $day < $signsStart[$month] ? $signs[$month - 1] : $signs[$month];
}
...
// TODO: СТРОКИ 76-103 (А где это задание? Сделайте скрипт-гороскоп. Внутри него хранится массив гороскопов на несколько дней вперед для каждого знака зодиака. По заходу на страницу спросите у пользователя дату рождения, определите его знак зодиака и выведите предсказание для этого знака зодиака на текущий день.
...
if (isset($_POST['a']) and isset($_POST['b']) and isset($_POST['c'])) {
    ...
    $disc = $b * $b - 4 * $a * $c;
    if ($disc > 0) {
        // TODO: ИСПРАВЛЕНО квадратный корень из $disc потерялся
        $x1 = round((-($b) + sqrt($disc)) / (2 * $a), 2);
        echo "x1 = $x1<br>";
        $x2 = round((-($b) - sqrt($disc)) / (2 * $a), 2);
        echo "x2 = $x2<br>";
    } elseif ($disc === 0) {
        // TODO: ИСПРАВЛЕНО все падает с фатальной ошибкой
        $x = round((-$b) / (2 * $a), 2);
        echo "x = $x<br>";
    } else {
...
// TODO: (В предыдущем задании не совсем поиск делителей. Простые множители -- это не все делители числа)Обсудить. По-хорошему можно было обойтись одной функцией по поиску делителей здесь и в задании выше.
...
if (isset($_POST['firstNODnum']) and isset($_POST['secondNODnum'])) {
    ...
    // TODO: ИСПРАВЛЕНО лучше использовать встроенную функцию сравнения массивов array_intersect
...
//        }
//    }
    $resultNOD = array_values(array_intersect($firstArr,$secondArr));
    echo 'НОД = ' . $resultNOD[0] . '<br><br>';
}
...
// TODO: ИСПРАВЛЕНО код выдает неверный результат
echo $forms['form17'];
