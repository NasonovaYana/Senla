...
// todo ИСПРАВЛЕНО (Опечатка .-.) это опечатка $timeStart.'<br>'?
$time = (int)(($timeNow - $timeStart) / 3600);
echo $time . '<br><br>';
...
// todo ИСПРАВЛЕНО cal_days_in_month - это функция из дополнительного модуля php. Реши эту и следующую задачу стандартными встроенными функциями
$daysCurrentMonth = date('t');
echo $daysCurrentMonth . '<br>';
//Задайте год (4 цифры), а скрипт определяет високосный ли год.
$year = 2021;
$date = mktime(0, 0, 0, 2, 1, $year);
$daysFeb = date('t', $date);
if ($daysFeb == 29) {
...
$mn = (int)date('m', $time);
// todo ИСПРАВЛЕНО нужен месяц
echo '31.12.2025 - ' . $month[$mn] . '<br><br>';
//Сравнение дат:
...
// todo ИСПРАВЛЕНО Выведи, даты которые должны были получиться в задании на экран.
$date = date_create('2025-12-31');
$date = date_modify($date, '+2 days');
echo date_format($date, 'j.m.y') . '<br>';
...
echo date_format($date, 'j.m.y') . '<br>';
$date = date_modify($date, '-3 days');
echo date_format($date, 'j.m.y') . '<br><br>';
...
// todo ИСПРАВЛЕНО посмотри, нужно ли что передавать в date_create, чтобы получить текущую дату
$day = date_create();
